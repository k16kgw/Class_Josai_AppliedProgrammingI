...
- コミュニティ検出を使って，強く結び付いた都道府県のクラスターを探す
...
| 8 | コミュニティを検出する | 強く結び付いた都道府県のクラスターを探す |
...
コミュニティ検出では，単にエッジの重みが大きい都道府県を集めるのではなく，次の2つを比較する．

1. 実際のネットワークで，同じグループ内にどれだけ強いエッジがあるか
2. 各ノードの重み付き次数を保ったまま，エッジが偶然に接続したとき，グループ内にどれだけの重みが期待されるか

実際のグループ内の結び付きが，偶然に期待される結び付きより強ければ，その分割にはコミュニティ構造があると考える．
...
| $c_i$ | ノード$i$が属するコミュニティ | 都道府県$i$のグループ番号 |
| $\delta(c_i,c_j)$ | $i$と$j$が同じコミュニティなら1，異なれば0 | 同じグループの組だけを合計する |
...
$Q$が大きいほど，同じコミュニティ内に期待以上の強いエッジが集まっている．
...
今回は，自分でコミュニティ検出のアルゴリズムを一から実装するのではなく，Pythonライブラリ`NetworkX`のコミュニティ検出モジュール`nx.community`を使用する．

コミュニティの検出には`nx.community.greedy_modularity_communities()`，検出結果のモジュラリティの計算には`nx.community.modularity()`を使う．

`greedy_modularity_communities()`は，モジュラリティを高めるコミュニティの統合を繰り返す**貪欲法**を実行する．

| 手順 | 処理 |
| --- | --- |
| 1 | 最初は各ノードを1つずつ別のコミュニティとする |
| 2 | 2つのコミュニティを統合した場合のモジュラリティの変化$\Delta Q$を調べる |
| 3 | $\Delta Q$が最も大きくなる2つを統合する |
| 4 | モジュラリティがそれ以上改善しなくなるまで統合を繰り返す |

例えば，東京都と神奈川県を統合したときのモジュラリティの増加が，他のどの統合よりも大きければ，その段階では東京都と神奈川県を同じコミュニティにする．
次の段階では，そのコミュニティを1つのまとまりとして，別の都道府県やコミュニティとの統合を検討する．

```{tip} 貪欲法の特徴
貪欲法は各段階で最も良い統合を選ぶため，計算が比較的速い．

一方，一度統合したコミュニティを後から分割してやり直すことはない．
...
したがって，エッジが「あるか，ないか」ではコミュニティを区別しにくい．

そこで，往復移動者数をエッジの重みとして使い，次のような都道府県の組を同じコミュニティにまとめる．
...
````{note} 演習8：人口移動ネットワークのコミュニティを検出する
