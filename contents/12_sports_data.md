...
| 項目 | 内容 |
| --- | --- |
| 試合 | 2013年11月3日 Tromsø IL（トロムソ・アイエル）対Strømsgodset IF（ストレームスゴトセト・アイエフ） |
...
したがって，`x_pos`は右方向へ，`y_pos`は上方向へ進むほど大きくなる．

このデータにはアウェイチームの位置も含まれていない．
...
````{dropdown} 利用条件の原文と授業での対応を確認する

**確認日：2026年7月4日**

[Alfheimデータセット公式ページ](https://datasets.simula.no/alfheim/)の「Randomized sensor tags」には，次のように記載されている．
...
授業での利用方法を原文の条件と対応させると，次のようになる．

| 公式ページの条件 | この授業での対応 |
| --- | --- |
| 非商用研究目的に限る | 大学の授業内で，データ取得・前処理・可視化手法を学ぶ目的に限って使用する |
| 選手を再識別しない | 匿名化された`tag_id`をそのまま使用し，氏名や背番号との対応を調べない |
| 選手・クラブのプロファイルを作らない | 個人やクラブの能力評価・順位付けを行わず，30秒間の座標を可視化手法の例として扱う |
| 利用目的を限定する | 作成したCSV・図・動画を授業と非商用の学習目的以外に転用しない |

以上から，本講義で定めた使用方法は，公式ページに記載された利用条件に沿っていると確認できる．
ただし，授業外での公開，別目的への転用，商用サービスへの利用を行う場合は，その時点の公式ページを再確認し，判断できない場合は提供者へ問い合わせること．
````
...
データを作成できなかった場合は，WebClassで授業内に限定して配布する`alfheim_tracking_5fps_csv.zip`をダウンロード・解凍し，`data/processed`に配置すること．
配布ファイルを授業外へ再配布しないこと．
...
```{tip} Notebookで動画を作成してよいか
今回の150フレーム程度のGIFであれば，Notebookで作成して問題ない．
セルごとに準備，更新，保存を確認できるため，動画作成の仕組みを学びやすい．

ただし，長時間・高fpsの動画は処理時間とファイルサイズが大きくなる．
Notebook自体へ全フレームを埋め込まず，GIFを別ファイルとして保存すること．
また，再利用や自動実行が必要になった場合は，完成した処理をPythonファイルへ移すとよい．
```
