...
第10回は，都道府県間の人口移動をネットワークとして可視化した．
...
今回は，データの**時間的な順序**に注目する．

### 今回の位置づけ

経済データには，物価，賃金，為替，株価，消費など，時間に沿って変化するデータが多い．
このようなデータを**時系列データ**として扱うと，長期的な方向，周期的な変動，短期的な変化を区別して考えることができる．

題材には，総務省統計局の**消費者物価指数** (CPI: Consumer Price Index)を使う．
...
| 2 | CPIと指数データを理解する | 基準年と系列の意味を確認する |
...
**時系列データ**とは，同じ対象について，異なる時点で繰り返し観測したデータである．
...
一方，同じ時点で複数の対象を比較したデータは**横断面データ**と呼ばれる．
例えば，2025年の47都道府県の人口を並べた表は横断面データである．

| 観点 | 時系列データ | 横断面データ |
| --- | --- | --- |
| 主な違い | 観測時点が異なる | 観測対象が異なる |
| 順序 | 時間順に意味がある | 行の順序は通常，本質的ではない |
| 主な問い | いつ，どのように変化したか | 対象間にどのような違いがあるか |

### 時間順に並べることの重要性

時系列データでは，行の順序が計算結果に影響する．

`shift()`，`diff()`，`rolling()`，`pct_change()`は，現在の行と前後の行を使って計算する．
年月が順不同のままでは，「1行前」が「1か月前」とは限らず，誤った結果になる．
...
この式は，時系列の見方を整理するための単純な加法モデルである．
現実のデータが常に完全に3つへ分解できるという意味ではない．
...
| --- | --- | --- |
| ラグ | $y_{t-k}$ | $k$期前の値 |
| 差分 | $\Delta y_t=y_t-y_{t-1}$ | 前期から何ポイント変化したか |
| 移動平均 | $\frac{1}{k}\sum_{i=0}^{k-1}y_{t-i}$ | 短期変動をならした動き |
| 前年同月比 | $\left(\frac{y_t}{y_{t-12}}-1\right)\times100$ | 1年前の同じ月から何%変化したか |
...
消費者物価指数は，家計が購入するモノやサービスの価格を総合した指数である．
...
「生鮮食品を除く総合」は，その影響を除いて物価の基調を見るために使われ，コアCPIと呼ばれる．

### 指数データと基準年

指数は，ある基準となる期間の値を100として，ほかの時点の水準を相対的に表したものである．

今回使用する2020年基準のCPIでは，**2020年の年平均**が100である．
...
月次の経済データには，季節によって繰り返す変動が含まれることがある．
...
統計ダッシュボードAPIの公式メタ情報では，`@isSeasonal`の`"1"`が原数値，`"2"`が季調値を表す．
この対応は値から推測せず，データ取得後にメタ情報APIで確認する．具体的な確認方法は演習2の後のdropdownで扱う．

```{tip} 系列を混ぜない
原数値と季節調整値を区別せずに表へ入れると，同じ指標・同じ年月が重複する．
平均やグラフを作る前に，どちらの系列を使うか確認すること．
```

---

## 統計ダッシュボードAPIからデータを取得する
...
`@time`は，`19700100`のように西暦4桁，月2桁，`00`の順で記録されている．
...
1. `@isSeasonal`には`"1"`と`"2"`があるか
2. 原数値と季節調整値では，観測開始時点が同じとは限らないことを説明できるか
3. 1行が「指標・系列種別・年月」の1つの観測値に対応しているか
````

次のdropdownを開き，`@isSeasonal`のコードと名称の対応を公式メタ情報から確認すること．
ここで作成する`SEASONAL_NAMES`は，演習3で使用する．

````{dropdown} `@isSeasonal`のコードを自分で確認する

### コードの意味を推測しない

`getData`のレスポンスには，`@isSeasonal`の値として`"1"`や`"2"`が記録されているが，その値だけを見ても意味は分からない．
...
1. 重複行数は0か
2. 総合・原数値に欠けている月はないか
3. 原数値と季節調整値の開始年月は同じか
4. ソート前に`shift()`を使ってはいけない理由を説明できるか
````
...
`src/plot_cpi_monthly_change.py`を作成し，WebClass「第11回課題」問5から提出せよ．
...
`src/plot_cpi_three_series.py`を作成し，WebClass「第11回課題」問6から提出せよ．
