...
前回は，第5回で取得した気象庁の天気予報JSONから，`timeSeries[0]` の天気，風，波を取り出して表形式CSVを作成した．
今回は，同じJSONの中に入っている複数の表を扱い，結合，日付処理，カテゴリ作成，集計用データセットの作成を学ぶ．
...
データの前処理IIとして，複数の表を組み合わせて分析用データセットを作成する方法を学ぶ．
...
- 地域別・天気カテゴリ別の集計データを作成できる

### 準備

<span style="color:red">今回も第5回で作成したフォルダ `5` 内で作業を続ける．</span>
...
4. 第6回で `data/raw/jma_tokyo_weather_raw_table.csv` を作成していない場合は，第6回の `src/make_raw_weather_table.py` を実行する．
...
第6回では，1つの時系列から1つの表を作ることを中心に扱った．
第7回では，1つのJSONの中に入っている複数の表を組み合わせ，分析目的に合う形へ変換する．
